...
def process_entry(data_row, prompt_name, input_keys, dis_res_num, max_retries: int = 5):
    ...
    # try:
    #     keep_k = random.randint(0, len(entry["keywords"]) * 2)
    #     keep_k = 1 if keep_k > len(entry["keywords"]) else keep_k
    #     entry["keywords"] = random.sample(entry["keywords"], keep_k)
    #     # keep_i = random.randint(0, 1)
    #     # entry["instruction"] = random.sample(entry["instruction"], keep_i)
    # except Exception as e:
    #     print(e)
    ...
    for i in range(distill_results_num):
        ...
        if prompt_res_val:
            data_row['distill_results'].append(prompt_res_val)
            data_row['distill_prompt'].append(prompt)
            data_row['new_source'] = "distill_eval_ifeval"
...
# def produce_data(data_rows, input_keys, prompt_name, output_path):
#     # entries = [{k: row[k] for k in input_keys} for row in data_rows]

#     with ThreadPoolExecutor(max_workers=4) as executor:
#         future_to_entry = {executor.submit(process_entry, data_row, prompt_name, input_keys): data_row for data_row in data_rows}#____________________________________________

#         for id, future in tqdm(enumerate(as_completed(future_to_entry)), desc='数据生产进度', total=len(data_rows)):
#             data_row = future.result()
#             future_to_entry[future] = data_row#__________________________
#             if not data_row['prompt_result']:
#                 print('[ERR] prompt_result is None')
    
#     with open(output_path, "w", encoding="utf-8") as f:
#         for item in data_rows:
#             f.write(json.dumps(item, ensure_ascii=False) + "\n")#__________________________
#     print('[Done] produce_data !')
...
if __name__ == '__main__':
    ...
    input_path = "/mnt/volumes/base-cv-ali-sh/chennuo5/data/data_dpo/0616/3_below_threshold_input.jsonl"
    output_path = "/mnt/volumes/base-cv-ali-sh/chennuo5/data/data_dpo/0616/3_below_threshold_raw.jsonl"
    input_keys = ["session","assistant"]
    prompt_name = "better_answer_gen"
    batch_size = 3500
